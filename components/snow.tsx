...
interface SnowParticle {
  ...
  export function Snow({
    ...
    return (
      <>
        ...
        {sleighEnabled ? (
          <SantaSleigh enabled={sleighEnabled} minDelay={sleighDemo ? 800 : sleighMinDelay} maxDelay={sleighDemo ? 1600 : sleighMaxDelay} size={sleighDemo ? Math.max(300, sleighSize) : sleighSize} demo={sleighDemo} />
        ) : null}
      </>
    );
  }
  ...
  export function SantaSleigh({ enabled = true, minDelay = 8000, maxDelay = 38000, size = 200, demo = false }: SantaProps) {
    ...
    useEffect(() => {
      ...
      const candidates = ["Santa Claus Sleigh Ride.json", "sleige_ride.json", "Santa_Claus_Sleigh_Ride.json"];
      (async () => {
...
export function SantaSleigh({
  ...
  useEffect(() => {
    ...
    // Position the element absolutely but keep pointer events off
    el.style.position = "fixed";
    el.style.pointerEvents = "none";
    el.style.width = `${size}px`;
    el.style.height = `${size}px`;
    ...
    // Fetch the JSON animation from public folder (encode filename safely)
    const candidates = [
      "sleigh_ride.json",
    ];
    ...
    const flyOnce = () => {
      ...
      // Fixed path: off-screen left to off-screen right, high symmetric arc
      const vw = window.innerWidth;
      const vh = window.innerHeight;

      // Start and end X: fully off-screen left/right
      const sx = -size;
      const ex = vw;
      // Y: both start and end at about 60% down from top (slightly above center)
      const baseY = Math.round(vh * 0.6) - Math.round(size / 2);
      const sy = baseY;
      const ey = baseY;

      // Control point: peak of arc, near top center
      const midX = Math.round(vw / 2);
      const midY = Math.round(vh * 0.18); // 18% from top, matches screenshot

      el.style.left = `${sx}px`;
      el.style.top = `${sy}px`;

      // Play Lottie if loaded
      isFlyingRef.current = true;
      attemptPlay(20, 100);

      // Animate along the fixed arc
      const baseDuration = 2200;
      const duration = baseDuration + (vw / 1200) * 1800 + Math.random() * 1000;
      const startTime = performance.now();
    ...
    // Trigger an initial visible flight shortly after mount so users see the sleigh
    const initialTimeout = window.setTimeout(() => {
      flyOnce();
    }, 500);

    // watcher to clear quickly if Lottie ref becomes available
    const lottieRefWatcher = setInterval(() => {
      if (animRef.current) {
        clearInterval(lottieRefWatcher);
      }
    }, 200);
